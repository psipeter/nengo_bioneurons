...
def test_tuning_curves(Simulator, plt):
	...
	with nengo.Network(seed=network_seed) as network:
		...
		pre_bio = nengo.Connection(pre, bio,
			syn_sec={'apical': {'n_syn': n_syn, 'syn_type': 'ExpSyn', 'tau': [tau]}},
			seed=conn_seed)
...
def test_tuning_curves_automated(Simulator, plt):
	pre_neurons = 100
	bio_neurons = 10
	tau = 0.05
	tau2 = 0.02
	...
	with nengo.Network(seed=network_seed) as network:
		...
		stim_pre = nengo.Connection(stim, pre,
			synapse=tau)
		pre_bio = nengo.Connection(pre, bio,
			syn_sec={'tuft': {'n_syn': n_syn, 'syn_type': 'Exp2Syn', 'tau': [tau2, tau]}},
			seed=conn_seed)
		pre_lif = nengo.Connection(pre, lif,
			synapse=tau)
	...
	from nengo_bioneurons import test_rates
	test_rates(
		network,
		Simulator,
		sim_seed,
