...
def test_fb_gradual(Simulator):
	...
	T_inter_bios = [1.0]  # np.linspace(1.0, 0.0, 3)  # [1.0, 0.5, 0.0]
	T_bio_bios = np.ones(len(T_inter_bios)) - T_inter_bios

	H_dir = '/home/pduggins/nengo_bioneurons/nengo_bioneurons/tests/filters/'
	H_inter = 'inter_bahl_evolve_reg_10s'  # 'inter_bahl_2'  #  'inter_alif_3'  #
	H_bio = 'bio_bahl_evolve_reg_10s'  # 'bio_bahl_14'  #  'bio_alif_7'  #  
	...
	# ax.legend()
	# fig.savefig('plots/ff_inter_estimate_evolve')
	...
	ax.legend()
	fig.savefig('plots/fb_gradual_transition_evolve_estimates')
	...
	fig.savefig('plots/fb_gradual_transition_evolve_spikes')
	...
	ax.plot(sim.trange(), x_target, label='target', ls='--')
	ax.legend()
	fig.savefig('plots/fb_gradual_bio_bio_evolve_estimates')
	...
	ax.legend()
	fig.savefig('plots/fb_gradual_bio_bio_evolve_decoders')
